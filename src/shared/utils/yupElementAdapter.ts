...
export const createYupValidator = (field: string, schema: yup.ObjectSchema<any>, item: any) => {
  return (rule: InternalRuleItem, value: any, callback: (error?: string | Error) => void) => {
    // Wrap the async validation in a way that works with Element Plus
    Promise.resolve()
      .then(() => schema.validateAt(field, item.value))
      .then(() => callback())
      .catch((error) => {
